...
Type
  Tppreconform = Class(TForm)
    ...
    //procedure ScriptSet(Const Operand1: String; Var Operand2: String; Const Operand3: String);
  ...
  Private
    ...
    //
    S_CAPITALIZEREFDATA: AnsiString;          // will use caps for all data brought in from the pathin data file
    //
    ...
    //
    S_RECPOINTERFIF01: LongInt;            // S_RECPOINTER Value when we last wrapped up a FINDINFILE01
    S_STATEFOUNDFIF01: AnsiString;         // StateFound value upon prior exit of FINDINFILE01
    S_RECPOSITIONFIF01: Integer;           // RECPOSITION value as we last exited FINDINFILE01
    S_SPERRFIF01: Integer;                 // SPERR value when we last exited FINDINFILE01
    //
    ...
    //
    S_STATEERROR: AnsiString;               // F in this means there was NO error
    //
    ...
    //
    S_TARRAYDELIM: AnsiString;       // Text Array field separator value (exactly one byte)
    S_TARRAYPOINTER: Longint;        // Text Array Pointer (a packed decimal long integer)
    S_TARRAYMAX: Longint;            // Ea time Tarray commd executed, this is set to max num elmts in the text array
    //
    ...
    J_SKIPINFILES: AnsiString;   //  how many of the input REF files to simply bypass
...
begin
  ...
  for i := StartByte to Length(OperandArea) do
      ...
      if capturing = 'Y' then
        ...
        // if (WorkingWithinQuotes = 'Y') and (OperandArea [i] = '"') then WorkingWithinQuotes := 'N';
        //   if (WorkingWithinQuotes = 'Y') and (mybyte = '"') then WorkingWithinQuotes := 'N';
        //   if (WorkingWithinQuotes = 'N') and (mybyte = '"') then WorkingWithinQuotes := 'Y';

           if (WorkingWithinQuotes = 'Y') and (mybyte = '"') then
            begin
              WorkingWithinQuotes := 'N';
            end
           else
            begin
              if (WorkingWithinQuotes = 'N') and (mybyte = '"') then
               begin
                 WorkingWithinQuotes := 'Y';
               end
            end;


           if (mybyte = ' ') and (WorkingWithinQuotes = 'N') then
             begin
               //EndByte := EndByte - 1;
               Break;
             end
             ...
             begin
               ...
               if (mybyte <> ' ') or (WorkingWithinQuotes = 'Y') then EndByte := EndByte + 1;
                // garygarygarygary above line is NEW and immed below line was live code prvsly
               //if mybyte <> ' ' then EndByte := EndByte + 1;
               //if EndByte > Length(OperandArea) then Break;
             end
         end
      else
...
Label
  EDITEXIT, NOCOLON, QUOTEERR, OPERANDSSET, WRONGNUMOPERANDS, TROVEERR, SETOK, SETTARGETERR;
...
OPERANDSSET:
  ...
  //
     if copy(Operand1, 1, 5) = '%TEXT' then GOTO SETOK;
     if copy(Operand1, 1, 6) = '%PCKD1' then GOTO SETOK;
     if copy(Operand1, 1, 17) = '%REFCURRENTRECORD' then GOTO SETOK;
     if copy(Operand1, 1, 12) = '%TARRAYDELIM' then GOTO SETOK;
     if copy(Operand1, 1, 14) = '%TARRAYPOINTER' then GOTO SETOK;
     if copy(Operand1, 1, 11) = '%RECPOINTER' then GOTO SETOK;
     GOTO SETTARGETERR;
SETOK:
     GOTO EDITEXIT;
   end;
  ...
  //
  //    0) CAPITALIZEREFDATA exactly one operand must exist for this.  It is either a Y or an N
  ...
  //
  //    2) SKIPINFILES:    A number of input REF files to bypass... before doing any work
  //    exactly ONE operand supplies this bypass count
  if Command = 'SKIPINFILES' then
   begin
     if countofoperands <> 1 then GOTO WRONGNUMOPERANDS;
     // gary - validate the value of the ONE operand is usable as a count limiter
     GOTO EDITEXIT;
...
NOCOLON:
  ...
  //
SETTARGETERR:
  result :=3;     // SET Target field is not valid
  GOTO EDITEXIT;
  //
TROVEERR:
...
begin
  ...
  try
    List.LoadFromFile(J_PATHIN);    {Load filenames from user-provided file}
  except
    ShowMessage('Job failed when tyring to load list of REF files from PATHIN.');
  end;
  goto GONE;
NOL:
...
ISO_NOTDOINGQUOTES:
  ...
  //
  if infield = '%TARRAYDELIM' then outfield := S_TARRAYDELIM;
  //
  ...
  //
  if infield = '%LASTMODDT' then
   begin
     outfield := S_LASTMODDT;     //gary new 09/18/2024
   end;
  //
...
ISO_NOTRECPOINTER:
  ...
  outfield := TrimRight(ISOLOPER(startmark, S_CURRRECORD+' ', endmark));
ISO_NOTFWORCR:
  //
  //
  //
...
FIF_LFF_NOTNEEDED:
  ...
  S_RECPOSITION := S_RECPOSITIONFIF01;
...
FIFBEGIN:
  ...
  //
  //    gary - for Jan, 2024 mods, I'm turning off this next if stmt     DEC4,2024  GaryGaryGary
  //
  if (soughtdata = 'RECORDRANGE(next,end)') OR (soughtdata = 'RECORDRANGE(NEXT,END)') Then
    begin
      recnostart := S_STATEFOUNDRECPOINTER + 1;       // to be able to keep looking beyond place where last FIND was successful
    end;
  //
  //
  //
  ...
  try
    ...
    if (tempSL.Text <> '') AND (tempSL.Text <> '.') then
      begin
        ...
        S_RECPOINTER := 1;
        S_CURRRECORD := S_REFFILERECORD[0];
        S_RECPOSITION := P;
      end;
  finally
    tempSL.Free;
...
FIF00:
  ...
  //garygarygarygarygary APR 16 2025
    lasti := recnostart;
    For i := recnostart - 1 To recnoend - 1 Do  // THIS IS THE ORIGINAL ONE - PUTTING THIS BACK ON JAN262024
      ...
      If (P <> 0) And (P >= colstart) And (P <= colend) Then             //
        Begin
          ...
          if numofgoodfinds = numofgoodfindsneeded Then
             begin
               S_STATEFOUND := 'T';
               //S_RECPOINTER := i + 1;                  // pointer saved should not be plus 1... garygary
               S_RECPOINTER := i;
               //
               S_CURRRECORD := S_REFFILERECORD[i];         //
               //
               S_RECPOSITION := P;
               //S_STATEFOUNDRECPOINTER := i + 1;     // pointer saved should not be plus 1... garygary
               S_STATEFOUNDRECPOINTER := i;
               S_SPERR := 0;
               Break;
             end;
        End;
...
LABEL
 INCROTH1, INCREMENTEXIT, INCREMENTERR8;
Begin
  S_SPERR := 0;
  S_STATEERROR := 'F';         //  assume that no error will occur
  //
  if copy(Operand1, 1, 11) <> '%RECPOINTER' then GOTO INCROTH1;
  //
  ...
  //
  S_CURRRECORD := S_REFFILERECORD[S_RECPOINTER];    //garygarygaryenable
  //S_RECPOINTER := S_RECPOINTER + 1;
  GOTO INCREMENTEXIT;
  //
INCROTH1:
  if copy(Operand1, 1, 6) = '%PCKD0' then
   begin
     S_PCKD0 := S_PCKD0 + 1;
     GOTO INCREMENTEXIT;
   end;
  if copy(Operand1, 1, 6) = '%PCKD1' then
   begin
     S_PCKD1 := S_PCKD1 + 1;
     GOTO INCREMENTEXIT;
   end;
  if copy(Operand1, 1, 6) = '%PCKD2' then
   begin
     S_PCKD2 := S_PCKD2 + 1;
     GOTO INCREMENTEXIT;
   end;
  if copy(Operand1, 1, 6) = '%PCKD3' then
   begin
     S_PCKD3 := S_PCKD3 + 1;
     GOTO INCREMENTEXIT;
   end;
  if copy(Operand1, 1, 6) = '%PCKD4' then
   begin
     S_PCKD4 := S_PCKD4 + 1;
     GOTO INCREMENTEXIT;
   end;
  if copy(Operand1, 1, 6) = '%PCKD5' then
   begin
     S_PCKD5 := S_PCKD5 + 1;
     GOTO INCREMENTEXIT;
   end;
  if copy(Operand1, 1, 6) = '%PCKD6' then
   begin
     S_PCKD6 := S_PCKD6 + 1;
     GOTO INCREMENTEXIT;
   end;
  if copy(Operand1, 1, 6) = '%PCKD7' then
   begin
     S_PCKD7 := S_PCKD7 + 1;
     GOTO INCREMENTEXIT;
   end;
  if copy(Operand1, 1, 6) = '%PCKD8' then
   begin
     S_PCKD8 := S_PCKD8 + 1;
     GOTO INCREMENTEXIT;
   end;
  if copy(Operand1, 1, 6) = '%PCKD9' then
   begin
     S_PCKD9 := S_PCKD9 + 1;
     GOTO INCREMENTEXIT;
   end;
  //
  if copy(Operand1, 1, 14) = '%TARRAYPOINTER' then
   begin
     S_TARRAYPOINTER := S_TARRAYPOINTER + 1;
     GOTO INCREMENTEXIT;
   end;
  //
INCREMENTERR8:
...
LABEL
 DECREMENTEXIT, DECREMENTERR8, DECROTH1, DECREMENTGOOD;
Begin
  S_SPERR := 0;
  S_STATEERROR := 'F';         //  assume that no error will occur
  //
  if copy(Operand1, 1, 11) <> '%RECPOINTER' then GOTO DECROTH1;
  //
  ...
  S_RECPOINTER := S_RECPOINTER - 1;
  S_CURRRECORD := S_REFFILERECORD[S_RECPOINTER]; //garygarygary
  GOTO DECREMENTEXIT;
  //
DECROTH1:
  //
...
var
 ...
 //LI: LongInt;
LABEL
 COMPARENUM, COMPAREEXIT, COMPAREISEQUAL, COMPAREISGREATER, COMPAREISLESS;
Begin
  ...
  //
  //garygarygary temp
  if copy(Operand1, 1, 6) = '%TEXT3' then
     begin
       compareformat := compareformat;  // temp
     end;
  //
  //
...
COMPAREISEQUAL:
  ...
  //
COMPARENUM:
  if Operand1 = '%PCKD0' then NUMLEFT := S_PCKD0;
  if Operand1 = '%PCKD1' then NUMLEFT := S_PCKD1;
  if Operand1 = '%PCKD2' then NUMLEFT := S_PCKD2;
  if Operand1 = '%PCKD3' then NUMLEFT := S_PCKD3;
  if Operand1 = '%PCKD4' then NUMLEFT := S_PCKD4;
  if Operand1 = '%PCKD5' then NUMLEFT := S_PCKD5;
  if Operand1 = '%PCKD6' then NUMLEFT := S_PCKD6;
  if Operand1 = '%PCKD7' then NUMLEFT := S_PCKD7;
  if Operand1 = '%PCKD8' then NUMLEFT := S_PCKD8;
  if Operand1 = '%PCKD9' then NUMLEFT := S_PCKD9;
  if Operand1 = '%TARRAYMAX' then NUMLEFT := S_TARRAYMAX;
  //
  if Operand2 = '%PCKD0' then NUMRIGHT := S_PCKD0;
  if Operand2 = '%PCKD1' then NUMRIGHT := S_PCKD1;
  if Operand2 = '%PCKD2' then NUMRIGHT := S_PCKD2;
  if Operand2 = '%PCKD3' then NUMRIGHT := S_PCKD3;
  if Operand2 = '%PCKD4' then NUMRIGHT := S_PCKD4;
  if Operand2 = '%PCKD5' then NUMRIGHT := S_PCKD5;
  if Operand2 = '%PCKD6' then NUMRIGHT := S_PCKD6;
  if Operand2 = '%PCKD7' then NUMRIGHT := S_PCKD7;
  if Operand2 = '%PCKD8' then NUMRIGHT := S_PCKD8;
  if Operand2 = '%PCKD9' then NUMRIGHT := S_PCKD9;
  if Operand2 = '%TARRAYMAX' then NUMRIGHT := S_TARRAYMAX;
  //
  if NUMLEFT = NUMRIGHT then GOTO COMPAREISEQUAL;
  if NUMLEFT > NUMRIGHT then GOTO COMPAREISGREATER;
  if NUMLEFT < NUMRIGHT then GOTO COMPAREISLESS;
  //
  S_STATEERROR := 'T';           //  show that an error occurred
...
var
 ...
 OP2PACVAL: LongInt;
LABEL
 SETBP, SETNUM, SETFAIL, SETEXIT, TARRAYX, TARRAYZ;
Begin
  S_SPERR := 0;
  S_STATEERROR := 'F';         //  assume that no error will occur
  //
  ...
  //
  if Operand1 = '%TEXT4' then         // garygary temptemp
     begin
       S_REFFILENAME := S_REFFILENAME; // garygary temp stuff
     end;
  //
  //
  if Operand1 = '%TEXT0' then         // garygary temptemp
     begin
       S_REFFILENAME := S_REFFILENAME; // garygary temp stuff
     end;
  //
  //
  if (Operand1 = '%TEXT5') and (Operand2 = '%TEXT5') then         // garygary temptemp
     begin
       S_REFFILENAME := S_REFFILENAME; // garygary temp stuff
     end;
  //
  //
  if (Operand1 = '%TEXT1') and (Operand2 = '%TEXT1') then         // garygary temptemp
     begin
       S_REFFILENAME := S_REFFILENAME; // garygary temp stuff
     end;
  //
  //
  if (Operand1 = '%PCKD1') and (Operand2 = '%RECPOINTER') then         // garygary temptemp
     begin
       S_REFFILENAME := S_REFFILENAME; // garygary temp stuff
     end;
  //
  //
  if (Operand1 = '%RECPOINTER') and (Operand2 = '%PCKD1') then         // garygary temptemp
     begin
       S_REFFILENAME := S_REFFILENAME; // garygary temp stuff
     end;
  //
  //
  if Operand1 = '%PCKD1' then         // garygary temptemp
     begin
       S_REFFILENAME := S_REFFILENAME; // garygary temp stuff
     end;
  //
  //
  if Operand2 = '%TEXTARRAY(%TEXT6)' then         // garygary temptemp
     begin
       if S_TARRAYPOINTER > 1 then
        begin
           S_REFFILENAME := S_REFFILENAME; // garygary temp stuff
  ...
  OP2FMT := 'T';
  //
  if Operand2 = '%PCKD0' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_PCKD0;
   end;
  if Operand2 = '%PCKD1' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_PCKD1;
   end;
  if Operand2 = '%PCKD2' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_PCKD2;
   end;
  if Operand2 = '%PCKD3' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_PCKD3;
   end;
  if Operand2 = '%PCKD4' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_PCKD4;
   end;
  if Operand2 = '%PCKD5' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_PCKD5;
   end;
  if Operand2 = '%PCKD6' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_PCKD6;
   end;
  if Operand2 = '%PCKD7' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_PCKD7;
   end;
  if Operand2 = '%PCKD8' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_PCKD8;
   end;
  if Operand2 = '%PCKD9' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_PCKD9;
   end;
  if Operand2 = '%RECPOINTER' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_RECPOINTER;
   end;
  if Operand2 = '%TARRAYPOINTER' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_TARRAYPOINTER;
   end;
  if Operand2 = '%TARRAYMAX' then
   begin
    OP2FMT := 'P';
    OP2PACVAL := S_TARRAYMAX;
   end;
  //
  targetformat := 'T';                                           // assume it will be a text set
  if (copy(Operand1, 1, 5) = '%PCKD')
   or (copy(Operand1, 1, 11) = '%RECPOINTER')
   or (copy(Operand1, 1, 11) = '%TARRAYPOINTER')
    then             // added recopointer 5/25/2023
     begin
...
SETBP:
  ...
  //
  //
  if copy(WorkOperand2, 1, 29) <> '%TEXTARRAY(%REFCURRENTRECORD)' then GOTO TARRAYX;
  //
  workTSL := TStringList.Create;
  workTSL.text := StringReplace(S_CURRRECORD, S_TARRAYDELIM, #13#10, [rfReplaceAll]);   // Create an array from the user-suggested data
  S_TARRAYMAX := workTSL.count;
  if S_TARRAYPOINTER = 0 then S_TARRAYPOINTER := S_TARRAYMAX;  // if user asked for element 0, they mean that they want highest numbered element
  if S_TARRAYMAX < S_TARRAYPOINTER then
    begin
      workTSL.Free;
      goto SETFAIL;
    end;
  //
  For i := 0 to workTSL.count - 1 DO
    begin
      //showmessage('i='+inttostr(i)+' array element='+ workTSL[i]);
      if S_TARRAYPOINTER = (i + 1) then
        begin
          WORKRIGHT := workTSL[i];
        end;
    end;
...
TARRAYX:
  //
  if copy(WorkOperand2, 1, 16) <> '%TEXTARRAY(%TEXT' then GOTO TARRAYZ;
  //
  if copy(WorkOperand2, 1, 18) = '%TEXTARRAY(%TEXT0)' then WORKRIGHT := S_TEXT0;
  if copy(WorkOperand2, 1, 18) = '%TEXTARRAY(%TEXT1)' then WORKRIGHT := S_TEXT1;
  if copy(WorkOperand2, 1, 18) = '%TEXTARRAY(%TEXT2)' then WORKRIGHT := S_TEXT2;
  if copy(WorkOperand2, 1, 18) = '%TEXTARRAY(%TEXT3)' then WORKRIGHT := S_TEXT3;
  if copy(WorkOperand2, 1, 18) = '%TEXTARRAY(%TEXT4)' then WORKRIGHT := S_TEXT4;
  if copy(WorkOperand2, 1, 18) = '%TEXTARRAY(%TEXT5)' then WORKRIGHT := S_TEXT5;
  if copy(WorkOperand2, 1, 18) = '%TEXTARRAY(%TEXT6)' then WORKRIGHT := S_TEXT6;
  if copy(WorkOperand2, 1, 18) = '%TEXTARRAY(%TEXT7)' then WORKRIGHT := S_TEXT7;
  if copy(WorkOperand2, 1, 18) = '%TEXTARRAY(%TEXT8)' then WORKRIGHT := S_TEXT8;
  if copy(WorkOperand2, 1, 18) = '%TEXTARRAY(%TEXT9)' then WORKRIGHT := S_TEXT9;
  //
  workTSL := TStringList.Create;
  workTSL.text := StringReplace(WORKRIGHT, S_TARRAYDELIM, #13#10, [rfReplaceAll]);   // Create an array from the user-suggested data
  S_TARRAYMAX := workTSL.count;
  if S_TARRAYPOINTER = 0 then S_TARRAYPOINTER := S_TARRAYMAX;  // if user asked for element 0, they mean that they want highest numbered element
  if S_TARRAYMAX < S_TARRAYPOINTER then
    begin
...
TARRAYZ:
  ...
  if Operand1 = '%TARRAYDELIM' then S_TARRAYDELIM := WORKRIGHT;

  if (copy(Operand1, 1, 5) = '%TEXT') or (copy(Operand1, 1, 12) = '%TARRAYDELIM') then GOTO SETEXIT;
  //
  if Operand1 = '%REFCURRENTRECORD' then
    begin
      S_CURRRECORD := WORKRIGHT;
      //garygarygarygary trying this next line instead of the one after that
      S_REFFILERECORD[S_RECPOINTER] := S_CURRRECORD;
...
//
  ...
  if Operand1 = '%RECPOINTER' then                           //garygary experimental
    begin
      ...
      // APRIL 16 2025
      S_STATEFOUNDRECPOINTER := S_RECPOINTER;
      if S_RECPOINTER > 0 then S_STATEFOUNDRECPOINTER := S_RECPOINTER - 1;
      //S_STATEFOUNDRECPOINTER := StrToInt(WORKRIGHT);   garygarygarygarygary - why setting this here???
      //
      S_RECPOINTERFIF01 := S_RECPOINTER;                     //garygary experimental - keep FIF pointer in line with reg recpointer
      //
      GOTO SETEXIT;
    end;
  //
  if Operand1 = '%TARRAYPOINTER' then                           //garygary experimental
    begin
      S_TARRAYPOINTER := StrToInt(WORKRIGHT);
      GOTO SETEXIT;
    end;
  //
    GOTO SETEXIT;
...
Begin
  ...
  if (S_STATEGREATERTHAN = 'T') then
    begin
      ScriptGoto(Operand1, '', '');       // this just sets field S_GOTOgtXTSTMT to the STATEMENT NUMBER that we should bounce to
      GOTO IFGTEXIT;
    end;
  S_STATEERROR := 'T';         //  show that an error occurred
...
Label
  NOGO, REFFILESDONE, REFTOP, NEXT, SCRIPTSEVERE, KEEPONX, KEEPON1, GO, NOPOSTPROCESSAUTOREPORT, TROVEACCUMDONE, LeaveLMD, LeaveLMDT;
...
Begin
 ...
 //  //
  ...
  //
  J_SKIPINFILES := '0';
  J_LIMITINFILES := '0';    //garygarygary
  rfprocessed := 0;         //garygarygary
  //rfskipped := 0;
   ...
   begin
     ...
     if StringGrid1.Cells[3, i] = 'PATHIN:' then
       begin
         ...
         J_PATHINOPT := 'R';
         countofdots := 0;
         countofdots := OccurrencesOfChar(J_PATHIN, '.');
     ...
     if StringGrid1.Cells[3, i] = 'SKIPINFILES:' then J_SKIPINFILES := StringGrid1.Cells[4, i];
...
//  end;
  ...
  //
  //
  if REFFILESSL.Count <= StrToInt(J_SKIPINFILES) then goto REFFILESDONE; // user asked to SKIP more files than what exists
  //
  //
  if StrToInt(J_SKIPINFILES) > 0 then       // if have to skip the first N entries in REFFILESSL, do this stuff
     begin
       TEMPSL := TStringList.Create;
       FOR i := StrToInt(J_SKIPINFILES) to REFFILESSL.Count - 1 Do
         Begin
           TEMPSL.Add(REFFILESSL[i]);
         End;
       REFFILESSL.clear;
       REFFILESSL.Assign(TEMPSL);
       TEMPSL.Free;
     end;
  //
  //
REFTOP:
  ...
  //
  //if (J_SKIPINFILES <> '0') and (rfskipped < StrToInt(J_SKIPINFILES)) then
  // begin
  //   rfskipped := rfskipped + 1;
  //   goto SKIPTHISREFFILE;
  // end;
  //
  rfprocessed := rfprocessed + 1; //garygarygary
  //
  if (StrToInt(J_LIMITINFILES) > 0) and (rfprocessed > StrToInt(J_LIMITINFILES)) then goto REFFILESDONE;    //garygarygary
  //
  ...
  //
  if J_PATHINOPT = 'L' then                                              // for J_PATHINOPT = L  (where user has provided fully qualified list of REF files)
    begin                                                                //  entire data from the PATHIN filelist was put into REFFILESSL... so,
      J_PATHIN := ExtractFilePath(REFFILESSL[0]);                        //   we now need to split the  filename away from the path name
  ...
  //
  //
  //garygarygary Sep 18, 2024  -   adjust Last Modified date so it always takes up same num of bytes and all data lines up
  LMDPieces := TStringList.Create;
  Split(S_LASTMODDT, '/', LMDPieces) ;
  if LMDPieces.Count < 2 then GOTO LeaveLMD;
  if length(LMDPieces[0]) < 2 then LMDPieces[0] := '0' + LMDPieces[0];
  if length(LMDPieces[1]) < 2 then LMDPieces[1] := '0' + LMDPieces[1];
  S_LASTMODDT := '';
  //
  For i := 0 to LMDPieces.Count - 1 Do
   Begin
    S_LASTMODDT := S_LASTMODDT + LMDPieces[i];
    if (i = 0) or (i = 1) then S_LASTMODDT := S_LASTMODDT + '/';
   End;
 LeaveLMD:
  LMDPieces.Free;
  //
  //
  LMDPieces := TStringList.Create;
  Split(S_LASTMODDT, ' ', LMDPieces) ;          // split date/time stamp into it's three parts
  if LMDPieces.Count <> 3 then GOTO LeaveLMDT;  // if it is not exactly 3 parts... never mind
  if length(LMDPieces[1]) < 8 then LMDPieces[1] := '0' + LMDPieces[1];
  S_LASTMODDT := '';
  //
  For i := 0 to LMDPieces.Count - 1 Do
   Begin
 ...
 //     log.Write(DtNow(Now) + 'User canceled run instead of selecting Output folder.');
  ...
  //
  S_SPERR := 0;
  // apr 17 2025 garygarygarygarygary
  S_SPERRFIF01 := 0;
  //
